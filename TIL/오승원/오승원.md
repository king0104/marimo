...
- 효과적인 발표 준비 및 문서 정리 방법  
- 코드 디버깅 및 오류 해결 프로세스  
- API 연동을 위한 사전 논의와 협업 방식  
...
- 알림 3종 스와이프 애니메이션 삭제 기능 추가  
- 알림 뱃지 사라지는 애니메이션 추가  
- 알림 버튼 클릭 시 이동 라우터 추가  

<br>
### ❣️ Today I've Learned

- UI/UX 개선을 위한 애니메이션 적용 방법  
- 사용자 경험을 고려한 알림 인터랙션 설계  
- 라우팅을 활용한 알림 이벤트 처리 방식  
...
- pubspec.yaml에 이미지 에셋 경로를 추가해야 사용가능!
- 이미지 없으면 렌더링 안되면서 앱 멈춤
...
- 네비게이션바 속성을 사용하면 본문과 네비게이션바 영역이 분리된다.
<br>
...
- 청중이 모르는 내용도 좀 더 이해하기 쉽게 전달할 발표 자료와 내용을 구상해봐야겠다.
...
- 청중에게 잘 전달되려면, 소리와 시각자료, 그리고 발표의 3박자가 잘 맞아야 한다.
...
- 화면 크기별로 보이는 게 달라서, 에뮬레이터에서는 괜찮은데, 사용자 기기에서는 네비게이션바가 다르게 보일 수 있다.
