...
class _InsuranceScreenState extends State<InsuranceScreen> {
  ...
  Future<void> _fetchInsuranceInfo() async {
    ...
    try {
      ...
      final info = await InsuranceService.getInsuranceInfo(
        carId!,
        accessToken!,
      );
        ...
        switch (e.statusCode) {
          ...
          case 403:
            ...
            ScaffoldMessenger.of(
              context,
            ).showSnackBar(const SnackBar(content: Text('접근 권한이 없습니다.')));
  ...
  Future<void> _deleteInsurance() async {
    try {
      ...
      if (accessToken == null || carInsuranceId == null) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('삭제에 필요한 정보가 없습니다.')));
        return;
      }
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('보험 정보가 삭제되었습니다.')));
  ...
  Widget build(BuildContext context) {
    ...
    final bool showExpiryNotice =
        insuranceInfo != null && _isNearExpiration(insuranceInfo!['endDate']);
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  Text(
                    isInsuranceRegistered ? '내 자동차 마일리지 특약 관리' : '내 자동차 보험 관리',
                    style: TextStyle(
                      fontSize: 16.sp,
                      fontWeight: FontWeight.w500,
                    ),
